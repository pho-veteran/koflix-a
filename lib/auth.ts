...
export const signOut = async (): Promise<void> => {
  try {
    ...
    if (!logoutResponse.ok) {
      console.error('Failed to clear session:', await logoutResponse.text());
    }
...
export const sendVerificationCode = async (phoneNumber: string): Promise<string | null> => {
  try {
    ...
    localStorage.setItem('phoneAuthVerificationId', confirmationResult.verificationId);
    ...
    if (typeof error === 'object' && error && 'code' in error) {
      ...
      if (errorCode === 'auth/quota-exceeded') {
        toast.error("SMS quota exceeded. Please try again later or use email registration.");
      } else if (errorCode === 'auth/invalid-phone-number') {
        toast.error("The phone number is not valid. Please enter a valid number with country code.");
...
export const verifyCode = async (code: string): Promise<User | null> => {
  ...
  try {
    ...
    const storedPassword = localStorage.getItem('phoneRegistrationPassword');
    if (storedPassword) {
